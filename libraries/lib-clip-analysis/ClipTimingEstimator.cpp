...
{
ClipTimingEstimator::ClipTimingEstimator(const ClipInterface& clip)
...
void ClipTimingEstimator::DoOutput(const float* outBuffer, size_t mStepSize)
{
   assert(false);
}

int ClipTimingEstimator::Process()
{
   ...
   const auto processor = [&](SpectrumTransformer& transformer) {
      ...
      for (auto i = 0u; i < real.size(); ++i)
      {
         ...
         magSpec[i] = std::sqrt(std::pow(real[i], 2) + std::pow(imag[i], 2));
   ...
   while (bLoopSuccess && samplePos < numSamples)
   {
      ...
      bLoopSuccess = ProcessSamples(processor, buffer.data(), numNewSamples);
