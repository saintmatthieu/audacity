 ...
 * It can read and write to past and future samples, but does not check for
 * overlaps
 */
...
template <typename SampleT> class CircularSampleBuffer
{
public:
   ...
   void setSize(int n)
   {
      if (n > _allocatedSize)
      {
         ...
         _buffer =
            (SampleT*)std::realloc(_buffer, _allocatedSize * sizeof(SampleT));
...
public:
   void writeBlock(int startOffset, int n, const SampleT* sourceBlock)
   {
      _splitBlockOffsetFunction(
         startOffset, n, [=](int bufferOff, int sampleOff, int n) {
            std::copy(
               &sourceBlock[sampleOff], &sourceBlock[sampleOff] + n,
               &_buffer[bufferOff]);
         });
   }
   ...
   void readBlock(int startOffset, int n, SampleT* targetBlock) const
   {
      _splitBlockOffsetFunction(
         startOffset, n, [=](int bufferOff, int sampleOff, int n) {
            std::copy(
               &_buffer[bufferOff], &_buffer[bufferOff] + n,
               &targetBlock[sampleOff]);
         });
   }
...
private:
   ...
   int _position0 = 0;     // position of sample index 0 inside _buffer. This is
                           // where the next sample will be written to
