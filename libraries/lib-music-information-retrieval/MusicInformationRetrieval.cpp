...
constexpr auto bpmExpectedValue = 126.3333;
   ...
   double audioFileDuration, Experiment1DebugOutput* debugOutput)
{
      ...
      possibleNumTatums.begin(), possibleNumTatums.end(), [&](int numTatums) {
         ...
         for (auto i = 0; i < static_cast<int>(odfSamplesPerDiv); ++i)
         {
            ...
            if (p >= max)
            {
               ...
               break;
            std::rotate(
               pulseTrain.begin(), pulseTrain.begin() + pulseTrain.size() - 1,
               pulseTrain.end());
         }
         std::vector<int> shiftedPeakIndices(peakIndices.size());
         std::transform(
            peakIndices.begin(), peakIndices.end(), shiftedPeakIndices.begin(),
         ...
         std::transform(
            ...
            peakDistances.begin(), [&](int peakIndex) {
               const auto closestDiv = std::round(peakIndex / odfSamplesPerDiv);
         ...
         const auto score =
            std::inner_product(
               peakDistances.begin(), peakDistances.end(), peakIndices.begin(),
   ...
   struct Stuff
   {
      ...
      double finalScore;
      double likelihood;
      std::vector<int> xcorrIndices;
      std::vector<double> xcorrValues;
      double xcorrAvg;
   };
   std::map<double /*bpm*/, Stuff> bpmsAndScores;
   std::for_each(
      possibleNumBeats.begin(), possibleNumBeats.end(), [&](int numBeats) {
         ...
         // miss beats.
         auto harmonic = 1;
         Stuff& stuff = bpmsAndScores[candidateBpm];
         ...
         stuff.finalScore = likelihood * stuff.xcorrAvg;
      });
