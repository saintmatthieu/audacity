...
TEST_CASE("GetRocInfo")
{
   ...
   const std::vector<TestCase> testCases {
      ...
      TestCase { Samples { { false, 100. }, { true, 200. } }, 0.,
                 Expected { 1., 150. } },
...
TEST_CASE("UpdateHash")
{
   ...
   UpdateHash<bufferSize>(SquareWaveMirAudioReader {}, hash);
   REQUIRE(hash == 0.);
}
...
TEST_CASE("TatumQuantizationFitBenchmarking")
{
   ...
   // stricter classifier will yield the same results, only with less false
   // positives.
   ...
   std::ofstream summaryFile {
      std::string(CMAKE_CURRENT_SOURCE_DIR) +
      "/GetMusicalMeterFromSignalBenchmarkingOutput/summary.txt"
   };
   ...
   // inadverted bug, and if it is deliberate, should be well justified.
