                           ...
                           double* cutlineEnd /* = NULL */) const
{
   for (const auto &cutline: mCutLines)
   {
      if (fabs(GetSequenceStartTime() + cutline->GetSequenceStartTime() - cutLinePosition) < 0.0001)
      {
         ...
         if (cutlineEnd)
            *cutlineEnd = startTime + cutline->SamplesToTime(
                                         cutline->GetVisibleSampleCount());
         return true;
      }
   }
...
{
   double endThis =
      GetRate() * GetPlayStartTime() + GetVisibleSampleCount().as_double();
...
{
   return GetNumSamples() - TimeToSamples(mTrimRight) -
          TimeToSamples(mTrimLeft);
}
