...
bool WaveClip::GetFloatsCenteredAroundTime(
   ...
   bool mayThrow) const
{
   ...
   const auto len = numSideSamples * 2 + 1u;
   const auto maybeNegativeStart = TimeToSamples(t) - sampleCount { numSideSamples };
   const auto effectiveStart = std::max(maybeNegativeStart, sampleCount { 0 });
   const auto maybeOutOfBoundEnd = maybeNegativeStart + len;
   ...
   std::fill(buffer + len - effectiveLen, buffer + len, 0.f);
   if (effectiveLen == 0)
      return false;
   const auto offsetBuffer = reinterpret_cast<char*>(buffer + numLeadingZeros);
   return GetSamples(
...
{
   return GetFloatsCenteredAroundTime(t, iChannel, &value, 0u, mayThrow);
}
...
{
   const sampleCount ts { t * mRate + 0.5 };
   if (ts <= GetPlayStartSample())
      return false;
   // Not using `GetPlayEndSample()` directly, because it doesn't take
   // `GetAppendBufferLen` in considration, meanwhile this implementation of
   // this method before we introduced stretching did ...
   const auto playEndSampleWithAppendBuffer =
      GetPlayStartSample() +
      sampleCount(
         (GetVisibleSampleCount() + GetAppendBufferLen()).as_double() *
            GetStretchRatio() +
         0.5);
   return ts < playEndSampleWithAppendBuffer;
}
