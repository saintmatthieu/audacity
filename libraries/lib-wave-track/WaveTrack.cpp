...
namespace {
struct GainAndPan : ClientData::Cloneable<> {
   GainAndPan() = default;
   ...
   static const GainAndPan &Get(const WaveTrack &track);
...
gainAndPanFactory{ [](auto &) { return std::make_unique<GainAndPan>(); } };

//! Copy can't be generated by default because of mutable members
GainAndPan::GainAndPan(const GainAndPan &other) {
   SetGain(other.GetGain());
   SetPan(other.GetPan());
}

GainAndPan::~GainAndPan() = default;

std::unique_ptr<ClientData::Cloneable<>> GainAndPan::Clone() const {
   return std::make_unique<GainAndPan>(*this);
}

GainAndPan &GainAndPan::Get(WaveTrack &track) {
   return track.GetGroupData().Track::ChannelGroupAttachments
      ::Get<GainAndPan>(gainAndPanFactory);
}

const GainAndPan &GainAndPan::Get(const WaveTrack &track)
{
   return Get(const_cast<WaveTrack &>(track));
}

float GainAndPan::GetGain() const
{
    ...
    , mpFactory(orig.mpFactory)
{
   ...
   for (const auto &clip : orig.mClipList.Get())
      mClipList.PushBack(std::make_unique<WaveClip>(*clip, mpFactory, true));
}
...
static Container MakeIntervals(const WaveClipHolders &clips)
{
   Container result;
...
{
   return MakeIntervals<ConstIntervals>( mClipList.Get() );
}

auto WaveTrack::GetIntervals() -> Intervals
{
   return MakeIntervals<Intervals>( mClipList.Get() );
}
...
{
   return GainAndPan::Get(*this).GetGain();
}

void WaveTrack::DoSetGain(float value)
{
   GainAndPan::Get(*this).SetGain(value);
}
...
{
   return GainAndPan::Get(*this).GetPan();
}

void WaveTrack::DoSetPan(float value)
{
   GainAndPan::Get(*this).SetPan(value);
}
...
bool WaveTrack::IsEmpty(double t0, double t1) const
{
   ...
   for (const auto &clip : mClipList.Get())
   {
      if (!clip->BeforeOrAtPlayStartTime(t1) && !clip->AfterOrAtPlayEndTime(t0)) {
                            ...
                            bool addCutLines, bool split)
{
   ...
   if (addCutLines)
   {
      ...
      {
         if (!clip->BeforeOrAtPlayStartTime(t1) && !clip->AfterOrAtPlayEndTime(t0) &&
               (clip->BeforeOrAtPlayStartTime(t0) || clip->AfterOrAtPlayEndTime(t1)))
         {
            addCutLines = false;
            break;
         }
   ...
   for (const auto &clip : mClipList.Get())
   {
      if (clip->BeforeOrAtPlayStartTime(t0) && clip->AfterOrAtPlayEndTime(t1))
      {
         // Whole clip must be deleted - remember this
         clipsToDelete.push_back(clip.get());
      }
      else if (!clip->BeforeOrAtPlayStartTime(t1) && !clip->AfterOrAtPlayEndTime(t0))
      {
         ...
         else
         {
            if (split) {
               ...
               if (clip->BeforeOrAtPlayStartTime(t0)) {
               ...
               else if (clip->AfterOrAtPlayEndTime(t1)) {
   ...
   if (!split && editClipCanMove)
   {
      ...
      for (const auto& clip : mClipList.Get())
      {
         if (clip->BeforeOrAtPlayStartTime(t1))
            clip->Offset(-(t1 - t0));
      }
   }
...
void WaveTrack::InsertSilence(double t, double len)
{
   ...
   if (mClipList.Empty())
   {
      ...
      for (const auto &clip : mClipList.Get())
      {
         if (clip->BeforeOrAtPlayStartTime(t))
            clip->Offset(len);
      }
   }
}
...
bool WaveTrack::GetErrorOpening() const
{
   for (const auto &clip : mClipList.Get())
                              ...
                              double* cutlineEnd)
{
   ...
   auto pClip = std::find_if( mClipList.Begin(), pEnd,
