...
bool AreAligned(const WaveClipPointers& a, const WaveClipPointers& b)
{
   ...
   return std::mismatch(a.begin(), a.end(), b.begin(), compare).first ==
          a.end();
}
...
static ProjectFileIORegistry::ObjectReaderEntry readerEntry { "wavetrack",
                                                              WaveTrack::New };
...
WaveTrackFactory::Create(sampleFormat format, double rate)
{
    ...
    , mpFactory(orig.mpFactory)
{
   ...
   for (const auto& clip : orig.mClips)
      mClips.push_back(std::make_unique<WaveClip>(*clip, mpFactory, true));
}
...
void WaveTrack::Merge(const Track& orig)
{
   orig.TypeSwitch(
      [&](const WaveTrack* pwt)
      {
         ...
         DoSetPan(wt.GetPan());
         // Copy attached data from orig.  Nullify data in this where orig had
         // null.
         Attachments& attachments = *this;
         attachments = *pwt;
      });
...
bool WaveTrack::LinkConsistencyFix(bool doFix, bool completeList)
{
   ...
   if (completeList)
   {
      ...
      if (
         static_cast<int>(linkType) == 1 || // Comes from old audacity version
...
{
   static const Track::TypeInfo info { { "wave", "wave", XO("Wave Track") },
                                       true,
...
wxString WaveTrack::MakeClipCopyName(const wxString& originalName) const
{
   ...
   for (auto i = 1;; ++i)
   {
      ...
      name = XC("%s.%i", "clip name template")
                .Format(originalName, i)
                .Translation();
   }
}
...
wxString WaveTrack::MakeNewClipName() const
{
   ...
   for (auto i = 1;; ++i)
   {
      ...
      name =
         XC("%s %i", "clip name template").Format(GetName(), i).Translation();
   }
}
...
void WaveTrack::ConvertToSampleFormat(
...
WaveTrack::Holder
WaveTrack::EmptyCopy(const SampleBlockFactoryPtr& pFactory, bool keepLink) const
{
...
Track::Holder WaveTrack::Copy(double t0, double t1, bool forClipboard) const
{
   ...
   for (const auto& clip : mClips)
   {
      ...
      else if (clip->CountSamples(t0, t1) >= 1)
      {
         ...
         auto newClip =
            std::make_unique<WaveClip>(*clip, mpFactory, !forClipboard, t0, t1);
...
//
/*! @excsafety{Weak} -- This WaveTrack remains destructible in case of
AudacityException. But some of its cutline clips may have been destroyed. */
...
)
{
   ...
   HandleClear(t0, t1, false, false);
   {
      ...
      if (preserve)
      {
         ...
         // Restore the split lines and trims, transforming the position
         // appropriately
         for (const auto& split : splits)
         {
            auto at =
               LongSamplesToTime(TimeToLongSamples(warper->Warp(split.time)));
            for (const auto& clip : GetClips())
            {
               if (clip->WithinPlayRegion(at)) // strictly inside
               {
                  ...
                  clip->GetPlayStartSample() == TimeToLongSamples(at) &&
                  split.right)
               {
                  ...
                  clip->GetPlayEndSample() == TimeToLongSamples(at) &&
                  split.left)
               {
                  attachRight(clip.get(), split.left.get());
                  break;
         ...
         for (const auto& clip : mClips)
         {
            ...
            for (auto it = cuts.begin(); it != cuts.end();)
            {
               ...
               if (cs >= st && cs <= et)
               {
                  ...
                  clip->GetCutLines().push_back(
                     std::move(*it)); // transfer ownership!
                  it = cuts.erase(it);
               }
               else
                  ++it;
...
namespace
{
WaveClipHolders::const_iterator FindClip(
   const WaveClipHolders& list, const WaveClip* clip, int* distance = nullptr)
{
   if (distance)
      *distance = 0;
   auto it = list.begin();
...
std::shared_ptr<WaveClip> WaveTrack::RemoveAndReturnClip(WaveClip* clip)
{
   ...
   {
      auto result =
         std::move(*it); // Array stops owning the clip, before we shrink it
      mClips.erase(it);
      return result;
   }
   else
...
bool WaveTrack::AddClip(const std::shared_ptr<WaveClip>& clip)
{
   ...
   // Uncomment the following line after we correct the problem of zero-length
   // clips
...
/*! @excsafety{Strong} */
void WaveTrack::HandleClear(double t0, double t1, bool addCutLines, bool split)
{
   ...
   if (addCutLines)
   {
      ...
      {
         if (
            !clip->BeforePlayStartTime(t1) && !clip->AfterPlayEndTime(t0) &&
...
void WaveTrack::SyncLockAdjust(double oldT1, double newT1)
{
   if (newT1 > oldT1)
   {
      ...
      // matters. GetEndTime() looks through the clips and may give us EXACTLY
      // the same value as T1, when T1 was set to be at the end of one of those
      // clips.
      if (oldT1 >= GetEndTime())
         return;
...
void WaveTrack::PasteWaveTrack(double t0, const WaveTrack* other)
{
   ...
   // mode, we must guess the behaviour the user wants.
   //
   ...
   if (insertDuration != 0 && insertDuration < 1.0 / mRate)
      // PRL:  I added this check to avoid violations of preconditions in other
      // WaveClip and Sequence methods, but allow the value 0 so I don't subvert
      // the purpose of commit 739422ba70ceb4be0bb1829b6feb0c5401de641e which
      // causes append-recording always to make a new clip.
      return;
   ...
   if (singleClipMode)
   {
      ...
      if (insideClip)
      {
         ...
         if (!editClipCanMove)
         {
            ...
            {
               if (
                  clip->GetPlayStartTime() > insideClip->GetPlayStartTime() &&
   ...
   for (const auto& clip : other->mClips)
   {
      ...
      if (!clip->GetIsPlaceholder())
      {
         ...
         if (pastingFromTempTrack)
            ...
            // considered to be new entities, thus named using "new" name
            // template
...
void WaveTrack::Disjoin(double t0, double t1)
{
   ...
   for (const auto& clip : mClips)
   {
      ...
      for (decltype(len) done = 0; done < len; done += maxAtOnce)
      {
         ...
         clip->GetSamples(
            (samplePtr)buffer.get(), floatSample, start + done, numSamples);
...
void WaveTrack::Join(double t0, double t1)
{
   ...
   {
      if (
         clip->GetPlayStartTime() < t1 - (1.0 / mRate) &&
   ...
   newClip = CreateClip(
      clipsToDelete[0]->GetSequenceStartTime(), clipsToDelete[0]->GetName());
...
bool WaveTrack::Append(
   constSamplePtr buffer, sampleFormat format, size_t len, unsigned int stride,
   sampleFormat effectiveFormat)
{
   return RightmostOrNewClip()->Append(
      buffer, format, len, stride, effectiveFormat);
}
...
sampleCount WaveTrack::GetBlockStart(sampleCount s) const
{
   for (const auto& clip : mClips)
   {
      ...
      {
         auto blockStartOffset =
            clip->GetSequence()->GetBlockStart(clip->ToSequenceSamples(s));
         return std::max(
            startSample, clip->GetSequenceStartSample() + blockStartOffset);
      }
   }
...
size_t WaveTrack::GetBestBlockSize(sampleCount s) const
{
   ...
   for (const auto& clip : mClips)
   {
      ...
      if (s >= startSample && s < endSample)
      {
         bestBlockSize = clip->GetSequence()->GetBestBlockSize(
            s - clip->GetSequenceStartSample());
         break;
      }
   }
...
size_t WaveTrack::GetMaxBlockSize() const
{
   ...
   {
      maxblocksize =
         std::max(maxblocksize, clip->GetSequence()->GetMaxBlockSize());
   }
   ...
   if (maxblocksize == 0)
   {
      ...
      maxblocksize = Sequence {
         mpFactory, SampleFormats { mFormat, mFormat }
      }.GetMaxBlockSize();
   }
   ...
   if (tag == "wavetrack")
   {
      ...
      for (auto pair : attrs)
      {
         ...
         if (attr == "rate")
         {
            ...
            if (
               !value.TryGet(dblValue) || (dblValue < 1.0) ||
               (dblValue > 1000000.0)) // allow a large range to be read
               return false;
         ...
         else if (this->WritableSampleTrack::HandleXMLAttribute(attr, value))
         {
            ...
            attr == "pan" && value.TryGet(dblValue) && (dblValue >= -1.0) &&
            (dblValue <= 1.0))
            DoSetPan(dblValue);
         else if (attr == "channel")
         {
            if (!value.TryGet(nValue) || !IsValidChannel(nValue))
               return false;
            ...
            attr == "sampleformat" && value.TryGet(nValue) &&
...
XMLTagHandler* WaveTrack::HandleXMLChild(const std::string_view& tag)
{
   ...
   // JKC... for 1.1.0, one step better than what we had, but still badly
   // broken. If we see a waveblock at this level, we'd better generate a
   // sequence.
   if (tag == "waveblock")
   {
      // This is a legacy project, so set the cached offset
...
WaveTrack::GetMinMax(double t0, double t1, bool mayThrow) const
{
...
float WaveTrack::GetRMS(double t0, double t1, bool mayThrow) const
{
   ...
   for (const auto& clip : mClips)
   {
      // If t1 == clip->GetStartTime() or t0 == clip->GetEndTime(), then the
      // clip is not inside the selection, so we don't want it. if (t1 >=
      // clip->GetStartTime() && t0 <= clip->GetEndTime())
      if (t1 >= clip->GetPlayStartTime() && t0 <= clip->GetPlayEndTime())
      {
         auto clipStart =
            clip->TimeToSequenceSamples(std::max(t0, clip->GetPlayStartTime()));
         auto clipEnd =
            clip->TimeToSequenceSamples(std::min(t1, clip->GetPlayEndTime()));
...
WaveClipHolders::const_iterator WaveTrack::_GetNextClip(double t) const
{
   return std::upper_bound(
      mClips.begin(), mClips.end(), t,
      [](double t, const WaveClipHolder& clip)
      { return t < clip->GetPlayStartTime(); });
}

sampleCount WaveTrack::_GetStretchedEndSample() const
{
   if (mClips.empty())
   {
      return 0;
   }
   const auto& lastClip = mClips[mClips.size() - 1u];
   const auto t0 = lastClip->GetPlayStartTime();
   const auto stretchedTime = lastClip->GetStretchedPlayDuration();
   return static_cast<sampleCount>((t0 + stretchedTime) * mRate + 0.5);
}

WaveClipHolders::const_iterator WaveTrack::_GetClipAtTime(double t) const
{
   const auto nextClipIt = _GetNextClip(t);
   if (nextClipIt == mClips.begin())
   {
      return mClips.end();
   }
   const auto clipIt = nextClipIt - 1;
   const auto& clip = *clipIt;
   const auto clipBeginTime = clip->GetPlayStartTime();
   const auto r = clip->GetTimeStretchRatio();
   const auto clipEndTime =
      clipBeginTime + (clip->GetPlayEndTime() - clipBeginTime) * r;
...
void WaveTrack::Reposition(double t)
{
   ...
   for (auto it = leftMostClipIt; it != mClips.end(); ++it)
   {
      ...
      if (clipStartTime > t)
      {
         auto segment = std::make_shared<SilenceSegment>(clipStartTime - t);
      ...
      processor.Reposition(t - clipStartTime);
   ...
   fillFormat fill, bool mayThrow, sampleCount* pNumWithinClips) const
{
   ...
   // clip, don't clear anything (because we won't have to). Otherwise, just
   // clear everything to be on the safe side.
   bool doClear = true;
   bool result = true;
   ...
   {
      if (
         start >= clip->GetPlayStartSample() &&
         start + len <= clip->GetPlayEndSample())
      {
         doClear = false;
         break;
      }
   ...
   for (const auto& clip : mClips)
   {
      ...
      if (clipEnd > start && clipStart < start + len)
      {
         ...
         if (
            !clip->GetSamples(
               (samplePtr)(((char*)buffer) + startDelta.as_size_t() * SAMPLE_SIZE(format)),
   ...
   sampleFormat effectiveFormat)
{
   for (const auto& clip : mClips)
   {
      ...
      if (clipEnd > start && clipStart < start + len)
      {
         ...
         clip->SetSamples(
            buffer + startDelta.as_size_t() * SAMPLE_SIZE(format), format,
            inclipDelta, samplesToCopy.as_size_t(), effectiveFormat);
         clip->MarkChanged();
      }
   }
}
...
sampleFormat WaveTrack::WidestEffectiveFormat() const
{
   ...
   return std::accumulate(
      clips.begin(), clips.end(), narrowestSampleFormat,
      [](sampleFormat format, const auto& pClip)
      {
         return std::max(
            format, pClip->GetSequence()->GetSampleFormats().Effective());
      });
}
...
{
   // The output buffer corresponds to an unbroken span of time which the
   // callers expect to be fully valid.  As clips are processed below, the
   // output buffer is updated with envelope values from any portion of a clip,
   // start, end, middle, or none at all. Since this does not guarantee that the
   // entire buffer is filled with values we need to initialize the entire
   // buffer to a default value.
   //
   // This does mean that, in the cases where a usable clip is located, the
   // buffer value will be set twice.  Unfortunately, there is no easy way
   // around this since the clips are not stored in increasing time order.  If
   // they were, we could just track the time as the buffer is filled.
   ...
   for (const auto& clip : mClips)
   {
      ...
      if ((dClipStartTime < endTime) && (dClipEndTime > startTime))
      {
         ...
         if (rt0 < dClipStartTime)
         {
            ...
            auto nDiff =
               (sampleCount)floor((dClipStartTime - rt0) * mRate + 0.5);
         ...
         {
            auto nClipLen =
               clip->GetPlayEndSample() - clip->GetPlayStartSample();

            if (nClipLen <= 0) // Testing for bug 641, this problem is
                               // consistently '== 0', but doesn't hurt to check
                               // <.
               return;

            // This check prevents problem cited in
            // http://bugzilla.audacityteam.org/show_bug.cgi?id=528#c11, Gale's
            // cross_fade_out project, which was already corrupted by bug 528.
            // This conditional prevents the previous write past the buffer end,
            // in clip->GetEnvelope() call. Never increase rlen here. PRL bug
            // 827:  rewrote it again
            rlen = limitSampleBufferSize(rlen, nClipLen);
            rlen = std::min(
               rlen, size_t(floor(0.5 + (dClipEndTime - rt0) / tstep)));
         }
...
WaveClip* WaveTrack::GetClipAtTime(double time)
{
   ...
   // exactly equal due to rounding errors. If "time" is the end time of the
   // first of two such clips, and the end time is slightly less than the start
   // time of the second clip, then the first rather than the second clip is
   // found by the above code. So correct this.
   if (
...
bool WaveTrack::CanOffsetClips(
   const std::vector<WaveClip*>& clips, double amount,
   double* allowedAmount /* = NULL */)
{
   ...
   for (const auto& c : mClips)
   {
      ...
      {
         if (
            c->GetPlayStartTime() < clip->GetPlayEndTime() + amount &&
            c->GetPlayEndTime() > clip->GetPlayStartTime() + amount)
         {
            ...
            {
               if (
                  c->GetPlayStartTime() - clip->GetPlayEndTime() <
                  *allowedAmount)
                  *allowedAmount =
                     c->GetPlayStartTime() - clip->GetPlayEndTime();
               if (*allowedAmount < 0)
            ...
            {
               if (
                  c->GetPlayEndTime() - clip->GetPlayStartTime() >
                  *allowedAmount)
                  *allowedAmount =
                     c->GetPlayEndTime() - clip->GetPlayStartTime();
               if (*allowedAmount > 0)
...
void WaveTrack::ExpandCutLine(
   double cutLinePosition, double* cutlineStart, double* cutlineEnd)
{
   bool editClipCanMove = GetEditClipsCanMove();
   ...
   if (pClip != pEnd)
   {
      ...
      if (!editClipCanMove)
      {
         ...
         {
            if (
               clip2->GetPlayStartTime() > clip->GetPlayStartTime() &&
...
void WaveTrack::MergeClips(int clipidx1, int clipidx2)
{
   ...
   if (!clip1 || !clip2) // Could happen if one track of a linked pair had a
                         // split and the other didn't.
...
/*! @excsafety{Weak} -- Partial completion may leave clips at differing sample
 * rates!
 */
...
void VisitBlocks(
   TrackList& tracks, BlockVisitor visitor, SampleBlockIDSet* pIDs)
{
...
void InspectBlocks(
   const TrackList& tracks, BlockInspector inspector, SampleBlockIDSet* pIDs)
{
...
BoolSetting EditClipsCanMove { L"/GUI/EditClipCanMove", false };
