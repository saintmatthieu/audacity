...
// TODO this is a copy of the function of the same name and signature in lib-utility.
...
GetBeatInfo(const RandomAccessReaderFn& sampleGetter, int sampleRate)
{
   ...
   while (sampleGetter(fBuff.data(), start, frameSize, false) == frameSize)
   {
      ...
      if (btrack.beatDueInCurrentFrame())
         result.beatTimes.push_back((start + frameSize / 2) / sampleRate);
      start += hopSize;
   }
