...
void TimeAndPitch::setup(int numChannels, int maxBlockSize)
{
   ...
   {
      d->inResampleInputBuffer[ch].setSize(
         _maxBlockSize +
  ...
  for (int i = 0; i < fftSize; ++i)
  {
   ...
   // force fft to allocate all buffers now. Currently there is no other way in
   // the fft class
...
{
   return std::max(
      0, int(std::ceil(d->exact_hop_a)) - _analysis_hop_counter +
...
{
   audio::simd::perform_parallel_simd_aligned(
      v, n, [](auto& a) { a = a - rint(a * 0.15915494309f) * 6.283185307f; });
}
...
{
   auto alpha =
      a_s / a_a; // this is the real stretch factor based on integer hop sizes

   // Create a magnitude array
   auto* mags = d->mag_flag.getPtr(0);
   vo::copy(
      d->mag.getPtr(0), mags, _numBins); // for stereo, just use the mid-channel
   ...
   for (int i = 0; i < num_peaks; ++i)
   {
      ...
      for (int ch = 0; ch < num_channels; ++ch)
         acc[ch][n] =
            acc[ch][n] +
            alpha * _unwrapPhase(p[ch][n] - p_l[ch][n] - fn_expChange_a) +
            fn_expChange_s;
   }
   ...
   for (int n = d->peak_index[0]; n > 0; --n)
   {
      for (int ch = 0; ch < num_channels; ++ch)
         acc[ch][n - 1] =
            acc[ch][n] - alpha * _unwrapPhase(p[ch][n] - p[ch][n - 1]);
   }
   ...
   for (int i = 0; i < num_peaks - 1; ++i)
   {
      ...
      for (int n = d->peak_index[i]; n < mid; ++n)
      {
         for (int ch = 0; ch < num_channels; ++ch)
            acc[ch][n + 1] =
               acc[ch][n] + alpha * _unwrapPhase(p[ch][n + 1] - p[ch][n]);
      }
...
/// process one hop/chunk in _fft_timeSeries and add the result to output
/// circular buffer
void TimeAndPitch::_process_hop(int hop_a, int hop_s)
{
   if (d->exact_hop_a != d->exact_hop_s)
   {
      if (_numChannels == 2)
         _lr_to_ms(
            d->fft_timeseries.getPtr(0), d->fft_timeseries.getPtr(1), fftSize);

      for (int ch = 0; ch < _numChannels; ++ch)
      {
         vo::multiply(
            d->fft_timeseries.getPtr(ch), d->cosWindow.getPtr(0),
            d->fft_timeseries.getPtr(ch), fftSize);
      ...
      {
         vo::calcMagnitudes(
            d->spectrum.getPtr(ch), d->mag.getPtr(ch),
            d->spectrum.getNumSamples());
         vo::calcPhases(
            d->spectrum.getPtr(ch), d->phase.getPtr(ch),
            d->spectrum.getNumSamples());
      }
      ...
      for (int ch = 0; ch < _numChannels; ++ch)
         vo::convertPolarToCartesian(
            d->mag.getPtr(ch), d->phase_accum.getPtr(ch),
            d->spectrum.getPtr(ch), d->spectrum.getNumSamples());
      d->fft.inverseReal(d->spectrum, d->fft_timeseries);

      for (int ch = 0; ch < _numChannels; ++ch)
         vo::constantMultiply(
            d->fft_timeseries.getPtr(ch), 1.f / fftSize,
            d->fft_timeseries.getPtr(ch), d->fft_timeseries.getNumSamples());
   ...
   { // stretch factor == 1.0 => just apply window
      for (int ch = 0; ch < _numChannels; ++ch)
         vo::multiply(
            d->fft_timeseries.getPtr(ch), d->sqWindow.getPtr(0),
            d->fft_timeseries.getPtr(ch), fftSize);
   }

   {
      float gainFact = float(
         _timeStretch *
         ((8.f / 3.f) / _overlap_a)); // overlap add normalization factor
      for (int ch = 0; ch < _numChannels; ++ch)
         d->outCircularBuffer[ch].writeAddBlockWithGain(
            _outBufferWriteOffset, fftSize, d->fft_timeseries.getPtr(ch),
            gainFact);

      if (normalize_window)
         d->normalizationBuffer.writeAddBlockWithGain(
            _outBufferWriteOffset, fftSize, d->sqWindow.getPtr(0), gainFact);
   }
   _outBufferWriteOffset += hop_s;
   _availableOutputSamples += hop_s;
}

void TimeAndPitch::setTimeStretchAndPitchFactor(
   double timeScale, double pitchFactor)
{
...
void TimeAndPitch::feedAudio(const float* const* input_smp, int numSamples)
{
   ...
   // The error accumulators will make the value fluctuate by one to reach
   // arbitrary scale ratios
   ...
   for (int ch = 0; ch < _numChannels; ++ch)
   {
      ...
      if (normalize_window)
      {
         constexpr float curve =
            4.f * 4.f; // the curve approximates 1/x over 1 but fades to 0 near
                       // 0 to avoid fade-in clicks
...
void TimeAndPitch::processPitchShift(
   float* const* smp, int numSamples, double pitchFactor)
{
