         ...
         if (iPair == endPair || leader != iPair->first) {
            ...
            leader->TypeSwitch(
               [&](WaveTrack &wn){
                  ...
                  // Whatever we are pasting is not merged with existing clips -
                  // this is a new desired behavior. Preserving the the
                  // boundaries within [t0, t1] therefore also isn't wanted.
                  constexpr auto preserveBoundaries = false;
                  wn.ClearAndPaste(t0, t1, *static_cast<const WaveTrack *>(src),
                                   preserveBoundaries, merge, &warper);
               },
               [&](LabelTrack &ln){
