   ...
   sampleCount start, sampleCount end, const TimeWarper &warper)
{
   ...
   auto maxBlockSize = leftTrack->GetMaxBlockSize();
   {
      ...
      while (sourceSampleCount < end) {
         ...
         leftTrack->GetFloats(GetTag{},(leftBuffer.get()), sourceSampleCount, blockSize);
         rightTrack->GetFloats(GetTag{},(rightBuffer.get()), sourceSampleCount, blockSize);
...
{
   if (mPreserveLength) {
      auto newLen = out->TimeToLongSamples(out->GetPlayDuration());
