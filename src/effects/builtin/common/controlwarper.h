 ...
 * The `warpedValue` of `ControlWarper` and the value of the control to be warped must be bound together and to nothing else.
 * `min` and `max` are the limits of the control.
 */
class ControlWarper : public QObject
{
    Q_OBJECT
    ...
    Q_PROPERTY(ControlWarpingType warpingType READ warpingType WRITE setWarpingType NOTIFY warpingTypeChanged FINAL)
...
public:
    explicit ControlWarper(QObject* parent = nullptr)
        : QObject(parent) {}

    Q_INVOKABLE void init();

    ControlWarpingType warpingType() const;
    void setWarpingType(ControlWarpingType type);
...
private:
    bool updateValue();
    bool updateWarpedValue();

    ControlWarpingType m_warpingType = ControlWarpingType::None;
    double m_value = 0.0;
